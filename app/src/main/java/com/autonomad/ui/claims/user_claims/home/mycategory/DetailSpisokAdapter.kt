...
import com.autonomad.utils.Constants.BASE_URL_IMAGES
import com.autonomad.utils.Methods
import com.squareup.picasso.Picasso
import kotlinx.android.synthetic.main.item_claims_spisok_detail.view.*

class DetailSpisokAdapter(
    private val detailSubViewModel: DetailSubModelView,
    val context: Context
) :
    RecyclerView.Adapter<DetailSpisokAdapter.DetailSpisokAdapterViewHolder>() {
    ...
    class DetailSpisokAdapterViewHolder constructor(
        ...
        fun bind(master: Master) {
            ...
            dataBinding.root.text_name.text = master.prof?.first_name
            dataBinding.root.text_specialization.text = master.description
            if (master.prof?.avatar?.isEmpty() == true) {
                dataBinding.root.ic_user.setImageResource(R.drawable.ic_profile_male);
            } else {
                Picasso.get()
                    .load("$BASE_URL_IMAGES${master.prof?.avatar}")
